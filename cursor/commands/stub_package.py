...
def stub_package(
    path: Path, *, include_docstrings: bool = True, include_private: bool = True
) -> str:
...
def main():
    ...
    parser.add_argument(
        "path", type=Path, help="Path to a .py file or package directory"
    )
    parser.add_argument(
        "--output", "-o", type=Path, help="Write output to file instead of stdout"
    )
    parser.add_argument(
        "--no-docstrings", action="store_true", help="Omit docstrings from stubs"
    )
    parser.add_argument(
        "--no-private",
        action="store_true",
        help="Skip _private functions/classes (keep __dunder__)",
    )
